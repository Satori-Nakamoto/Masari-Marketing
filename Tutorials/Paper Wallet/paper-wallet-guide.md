...
This option requires a moderate understanding of Linux, network security, and general computer knowledge.
...
5. Booting into a Linux distro (Linux Mint is recommended for people who are used to Windows OS), plug in your USB stick and navigate to it. Paranoid users can double check that the sha256sum is still the same by opening a terminal, navigating to the location of the ZIP file and typing `sha256sum masari-wallet-generator-master.zip`
